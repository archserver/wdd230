...
function displayForcast(data) {
    ...
    // Get the date so that I can select only the next few days
    const wcurrentDate = new Date();
    wcurrentDate.setHours(0,0,0,0);
    ...
    data.list.forEach(item => {
        ...
        const daydiff = (forecastDate - wcurrentDate) / 1000 / 60 / 60 / 24;

        if ((hour >= 9 && hour <= 15) && daydiff <= 1)
        {
