...
const linksURL = 'https://archserver.github.io/wdd230/scoots/data/equipment.json';
...
const displayEquipment = (equipmentlist) => {
    ...
    if(equipmentTable){
        ...
        let tableHTML = `<thead> <tr> <th colspan="6" scope="colgroup">Rental Pricing</th> </tr>
        <tr>
           <th scope="col"></th>
           <th scope="col">Max</th>
        ...
        for(const tier in equipmentlist){
            if(equipmentlist.hasOwnProperty(tier)){
                tableHTML += `<tbody><tr><th colspan="6">${tier.toUpperCase()}</th></tr>`;
                //for each item in each tier
                equipmentlist[tier].forEach(item => {
                    tableHTML += `<tr>
                        <td>${item.name}</td>
                        <td>${item.passangers}</td>
                        <td>${item.rhd}</td>
                        <td>${item.rfd}</td>
                        <td>${item.whd}</td>
                        <td>${item.wfd}</td>
